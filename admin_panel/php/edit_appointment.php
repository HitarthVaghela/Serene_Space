...
try {
    ...
    $therapists = $pdo->query("SELECT id, fullname FROM user_info WHERE user_type = 'Therapist'")->fetchAll(PDO::FETCH_ASSOC);
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    ");
    $check_stmt->execute([$therapist_id, $appointment_date, $appointment_time]);
    ...
    if ($existing_appointments > 0) {
        ...
        try {
            ...
            $update_stmt = $pdo->prepare("
                UPDATE appointment 
                SET appointment_date = ?, status = ?, therapist_id = ?, appointment_time = ? 
                WHERE appointment_id = ?
            ");
            $update_stmt->execute([$appointment_date, $status, $therapist_id, $appointment_time, $appointment_id]);
...
<form method="post">
    ...
    <label for="appointment_time">Appointment Slot:</label>
